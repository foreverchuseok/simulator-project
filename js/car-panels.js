...
/* 부품설계 221–223p. 외형만 구현: 표시 4, 1~4층 버튼, 50/110% 스위치.
...
function buildCarControls(parent,{floorY,frontZ,sideX}) {
  ...
  // 223p: 플랫폼의 하중 감지 볼트와 플랭크 전면 취부판에 고정된 두 스위치.
  ...
  box('loadBoltUpperPlate',0.86,0.012,0.13,metal,0,baseY-0.092,loadZ,load);
  for(const x of [-0.17,0.17])cyl(0.007,0.035,metal,x,baseY-0.086,loadZ,load);
  box('loadSwitchMountShelf',0.86,0.012,0.18,metal,0,baseY-0.222,0.17,load);
  ...
  for(const [i,threshold] of [50,110].entries()) {
    ...
    // 무부하 상태의 작은 접점 간격. 상부 감지볼트 길이로 두 기준을 구분한다.
    const boltBottom=baseY-(threshold===50?0.207:0.201),boltTop=baseY-0.09;
