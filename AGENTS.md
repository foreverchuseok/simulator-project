...
이 파일은 Claude Code, Google Antigravity, Codex, Cursor가 함께 따르는 프로젝트 공통 규칙이다.
...
- 작업 시작 전 [공유 로드맵](docs/ROADMAP.md)의 현재 단계와 다음 작업을 확인한다. 개발 순서는 로드맵에만 기록하며, 현재 사용자 요청을 우선한다.
...
- 이동케이블·종단 리미트 스위치(파이널·리미트·강제감속) 작업은 `docs/TRAVEL-CABLE-TERMINAL.md`를 먼저 읽는다. 좌우 배치와 공용 수치는 `index.html`의 **종단 안전장치 · 이동케이블 공용 계약** 블록이 유일한 원본이다.
...
- 공통 규칙은 이 파일이다. `CODEX.md`, `SKILL.md`, `.cursor/rules`는 만들지 않는다.
- `CLAUDE.md`와 `GEMINI.md`는 입구만 가리킨다. 규칙은 여기 적는다.
- Codex와 Cursor는 이 파일을 바로 읽는다.

### 도구 사용

- 특정 모델이나 한 도구만 고정하지 않는다. 작업 난이도, 사용 한도, 자료 형식에 따라 Claude Code, Google Antigravity, Codex, Cursor를 혼용한다.
- 네 도구 모두 일반 요청에서 기존 코드를 확인한 뒤 바로 분석·수정·검증한다. 구현을 시작할지 다시 묻지 않는다.
- 어느 도구든 사용자가 실행·구현을 말하면 바로 수행한다.
- 모델명과 요금제는 바뀔 수 있으므로 프로젝트 규칙에 고정하지 않는다.

### PLAN.md 운영

- 전체 개발 순서와 단계별 완료·인계 상태는 `docs/ROADMAP.md`에 공유한다. 단계가 바뀌거나 작업을 인계할 때 현재 상태와 다음 작업을 갱신한다.
- `PLAN.md`는 현재 작업 하나만 담는 로컬 임시 문서이며 Git에 올리지 않는다.
- 다른 PC에는 `PLAN.md`가 없을 수 있으므로, 재개에 필요한 핵심 상태는 로드맵에도 남긴다. 같은 작업 폴더의 다른 터미널은 해당 작업의 `PLAN.md`를 함께 참고한다.
- 네 도구 모두, 사용자가 `@PLAN.md`를 지정하거나 `PLAN.md`에 담아 달라고 명시한 경우에만 이 파일에 계획을 쓴다.
- 그 지시가 없으면 `PLAN.md`를 만들지 않고, 채팅에만 답하거나 바로 구현한다.
- 계획만 달라고 하면 지정한 곳에 적고 멈춘다. 구현할지 다시 묻지 않는다.
- 실행하라고 하면 `PLAN.md` 없이, 또는 사용자가 지정한 `PLAN.md`를 읽고 바로 구현한다.
- 파일이 존재한다는 이유만으로 현재 요청과 무관한 계획을 자동 실행하지 않는다.
- `PLAN.md`의 작업 목표·범위·순서·완료 기준은 사용자 지시 없이 교체하거나 변경하지 않는다.
- 실행 중에는 확인된 진행 상태·검증 결과·남은 문제만 갱신한다. 다른 작업 요청을 받더라도 기존 계획을 자동으로 덮어쓰지 않는다.
- 작업 완료 후에도 사용자 지시가 있을 때만 삭제하거나 다음 작업 내용으로 교체한다.
- 기본 형식은 다음과 같다.
...
- 최적화 단계 이후에도 부품·기능을 추가할 때 동일 조건의 성능 측정과 실제 모바일 조작 확인을 완료 기준에 포함한다. 기능 검증 통과만으로 모바일 품질까지 검증됐다고 보고하지 않는다.
...
- 이동케이블·종단 안전장치는 `node tools/verify_travel_cable.mjs`로 수치와 스크린샷을 함께 확인한다.
...
- `PLAN.md`: 사용자가 이 파일에 담아 달라고 지정했을 때만 쓰는 로컬 임시 계획. 영상·프레임 분석 메모는 `docs/`에 상시 문서로 두지 않고 여기 또는 `temporary/`에 둔 뒤 작업이 끝나면 지운다.
