...
현재 단계는 **1단계 — 웹 최적화와 기본 사용 경험 정리**이며, 로프 형상 재사용, 레일 브라켓·카·승장 고정 부품 묶음, 그림자 캐시와 레일 체결부 묶음의 PC 전후 측정·운행·Live Server 검증을 마쳤다. 다음은 기본 배경 단순화와 관찰 화면 구분이다. Galaxy Tab S7 FE / Chrome을 기준으로 실제 모바일 검증이 남아 있다. 전체 순서와 다음 작업은 [공유 로드맵](docs/ROADMAP.md)을 참고한다.
