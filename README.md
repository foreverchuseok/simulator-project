...
현재 단계는 **1단계 — 웹 최적화와 기본 사용 경험 정리**이며, 로프·고정부품·그림자 최적화에 이어 간단 배경과 전체 운행·부품 관찰 화면을 구현하고 PC 전후 측정·운행·Live Server 검증을 마쳤다. 다음은 모바일 사용 경험 정리다. Galaxy Tab S7 FE / Chrome을 기준으로 실제 모바일 검증이 남아 있다. 전체 순서와 다음 작업은 [공유 로드맵](docs/ROADMAP.md)을 참고한다.
