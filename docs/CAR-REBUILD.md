...
- 벽에서 관 중심까지 85mm. 후면·우측 유리는 직접 취부하지 않고 기존 금속 이음 기둥과 앞쪽 테두리에 고정판·지지봉·볼트를 둔다. 고정 취부 메시만 재질별로 묶는다. 손잡이 치수 원본은 함수 내 `hr`이며 `S`와 운행·로프·균형추는 유지한다.
- 사용자 선택 1번에 따라 측면 조작반 버튼 중심을 850→1050mm로 올렸다. 손잡이 윗면과 조작반 하단의 수직 간극은 72.5mm다. 버튼 7개·점자·방향은 유지한다.
- `node tools/verify_car_handrails.mjs`: 높이·지름·연속 이음·출입구 비간섭·조작반 간극·층별 추종·390px 터치 호출/도어 열림과 PC/세로 화면을 검증한다. 기존 `verify_car_panels.mjs` 전체 검사는 현재 코드에 없는 PM1/PM2 하중센서명을 참조해 실패하므로 이번 손잡이 검증과 구분한다. 실제 휴대폰 확인은 남아 있다.

### 카 상·하부 고정 배선 (2026-09-19)

- 사용자 표시 사진과 후속 정정에 따라 모서리·프레임의 검은 전선을 카탑 박스 아래에서 묶어 원형 보호 부시가 있는 큰 구멍 하나에 인입한다. 상세는 [CAR-CONTROLS.md](CAR-CONTROLS.md)의 최신 배선 계약을 따른다. 아래 안전장치의 과거 천장 횡단 경로는 이 계약으로 대체한다.
- 안전장치 리드는 글랜드를 나온 뒤 보강채널 밑으로 내려 빔 후면·우측 외곽을 따라 후면 모서리로 올라간다. 천장 뒤쪽과 왼쪽 토보드 밖을 돌아 박스 하부 공용 인입구에 연결하며, 가동 링크·센서 위치·피벗·로프는 유지한다.

### 에이프런·바이패스 경광등 (2026-09-19)

- 원본은 `js/car-underbody.js`다. `부품설계.pdf` 256페이지의 전면 판과 뒤쪽 지지 브라켓, 227페이지의 하향 사각 빨간 CC27 부저를 현재 카 치수에 맞춰 구성한다. 에이프런 수직부 750mm·하단 60° 절곡 35mm·폭 `S.DOOR_W+0.10`이며 카 실 바로 아래에 설치한다. 도면에 미기재된 외형 치수는 재구성 값이다.
- `carUnderbody`는 `carGrp` 자식이다. 경광등 Z는 실제 `carOverloadAssembly` 중심과 에이프런 사이에서 계산하며, 장착판·스페이서는 `carPlatform.userData.stringerX/bottomY`의 실제 보강채널 위치에서 파생한다. 기존 카 좌표·로프·균형추는 유지한다.
- 고장 메뉴의 **바이패스 / BYPASS**에서 해제·승장문·카문 중 하나를 선택한다. 활성화하면 INS로 전환되고 정상 호출·자동 도어·AUT 복귀·자동 구출운전을 차단한다. 이동 중 모드 변경과 두 문 동시 선택은 허용하지 않는다. 점검 ▲▼를 누르는 동안만 움직이며, 카문 바이패스도 두 문짝의 닫힘 위치를 별도로 확인한다. 우회하지 않은 문 위치/잠금이 풀리면 점검운전을 멈춘다.
- 바이패스 점검 이동 중 0.65초 주기로 빨간 렌즈를 0.28초 켜고 880Hz 삼각파 경보음을 반복한다. 음높이 변화는 없다. 정지·비상정지·해제·탭 숨김 시 경보를 끈다. Web Audio는 사용자 선택/점검 버튼 입력에서 활성화한다. 브라우저 음량은 실제 1m 거리의 55dB(A)를 보장하는 수치가 아니며, 접점은 시뮬레이터 위치/상태 판정이다.
- 검증: `node tools/verify_car_underbody.mjs`, Live Server `http://127.0.0.1:5500/index.html`. `.shot-car-underbody/report.json`과 장착·점등/소등·모바일 크기 PNG를 남긴다. 실제 모바일 조작·음색/음량 확인은 별도다.
